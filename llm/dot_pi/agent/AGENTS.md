...
- Each delegated agent runs in its own dedicated tab, not in the orchestrator's tab: create the tab (`herdr tab create`), start the agent into it, then close the empty root pane so the agent owns the tab (`pane_count == 1`).
- Continuation / follow-up tasks reuse an existing agent: search `herdr agent list` by `name` (the launch-time `--name <task-id>`); if it is alive (`idle`/`working`) send the next step to the same pane with `herdr pane run <pane_id> "<text>"`, and if it is gone restore its session (`agent_session.value`) with `pi --resume`/`pi --session <path>` (keeping the same `--name`). Create a dedicated tab only for genuinely new tasks.
...
4. Before spawning, search `herdr agent list` by `name` (= the launch-time `--name <task-id>`) for a prior agent to continue. For a continuation/follow-up: if it is alive (`idle`/`working`), send the next step to the same pane with `herdr pane run <pane_id> "<message>"` and create no new tab; if it is gone, restore its session (`agent_session.value`) with `pi --resume`/`pi --session <path>` (keeping the same `--name`). Only a genuinely new task proceeds to the next step.
5. Create a dedicated tab (`herdr tab create` in the focused workspace, capture `tab_id` and `root_pane_id`), start the child with `herdr agent start --tab <NEW_TAB> --no-focus` (no `--split`), then close the empty root pane (`herdr pane close <ROOT_PANE>`) so the agent owns the tab; follow the **Async handoff** procedure (wait, read, collect the report file).
...
- Continuation sends use `herdr pane run <pane_id> "<text>"` (send-text + Enter). `herdr agent send <name> <text>` only places text in the input box and does **not** press Enter, so the message never executes. **Pitfall:** if a follow-up is not picked up, confirm you used `pane run`, not `agent send`.
