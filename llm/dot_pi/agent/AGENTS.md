...
- Continuation / follow-up tasks reuse an existing agent: search `herdr agent list` by `name` (the launch-time `--name <task-id>`); if it is alive (`idle`/`working`), resolve the name to a pane and send the next step to the same pane with `herdr pane run` (send-text + Enter), and if it is gone restore its session (`agent_session.value`) with `pi --resume`/`pi --session <path>` (keeping the same `--name`). Create a dedicated tab only for genuinely new tasks.
...
4. Before spawning, search `herdr agent list` by `name` (= the launch-time `--name <task-id>`) for a prior agent to continue. For a continuation/follow-up: if it is alive (`idle`/`working`), resolve the name → pane_id via `herdr agent list`, then send the next step to the same pane with `herdr pane run` (send-text + Enter) using the by-name snippet documented in `herdr-orchestrator.md`; never use `herdr agent send` (it does not press Enter), and create no new tab. If it is gone, restore its session (`agent_session.value`) with `pi --resume`/`pi --session <path>` (keeping the same `--name`). Only a genuinely new task proceeds to the next step.
