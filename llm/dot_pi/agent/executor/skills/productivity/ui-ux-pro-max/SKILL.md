...
Run commands from this skill's directory so `scripts/search.py` resolves. Use the skill's absolute path (found via `find ~/.pi/agent/skills -name SKILL.md`) or let the agent resolve it. If `python` is not available, use `python3`.
